...
const parser = new Parser({
  timeout: 20000,
  headers: {
    'User-Agent': 'Mozilla/5.0 (compatible; CentralNoticias/3.4.1)',
...
app.get('/api/status',(_,res)=>{
  res.json({version:'3.4.1',now:new Date().toISOString(),modules:diagnostics});
});
...
app.get('/health',(_,res)=>res.json({ok:true,version:'3.4.1',now:new Date().toISOString()}));
app.get('*',(_,res)=>res.sendFile(path.join(__dirname,'public','index.html')));

app.listen(PORT,()=>{
  console.log(`Central de Notícias v3.4.1 ativa na porta ${PORT}`);
